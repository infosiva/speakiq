...
export function ConversationMode({ language, level, tutorName = 'Luna' }: Props) {
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden" style={{ height: 'calc(100dvh - 120px)', minHeight: '520px' }}>
        ...
        {turns.map((t, i) => (
          <div key={i} className={`flex flex-col gap-0.5 ${t.role === 'user' ? 'items-end' : 'items-start'}`}>
            ...
            {t.corrections && t.corrections.length > 0 && (
              <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg px-3 py-2 text-xs space-y-1 max-w-[75%]">
